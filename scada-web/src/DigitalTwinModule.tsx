...
const initialTwinLayout: TwinLayout = {
  ...
  stations: Object.fromEntries(Object.entries(buildStations()).map(([id, station]) => [id, {
    ...station,
    // Keep the same visible demo motion as the previous digital-twin screen.
    forward: station.opCode === 'OP3',
    reverse: station.opCode === 'OP4' || station.opCode === 'OP7',
    occupied: station.opCode === 'OP4',
  }])),
}

function useSharedTwinLayout() {
  const [layout, setLayout] = useState<TwinLayout>(() => {
    try {
      // v4 intentionally reseeds the restored motion demo after the earlier
      // static layout was saved under v3.
      const raw = localStorage.getItem('mes-digital-twin-layout-v4')
      if (!raw) return initialTwinLayout
      const stored = JSON.parse(raw) as Partial<TwinLayout>
      return {
        ...initialTwinLayout,
        ...stored,
        car: { ...initialTwinLayout.car, ...stored.car, relativeX: clampRelativeX(stored.car?.relativeX ?? initialTwinLayout.car.relativeX) },
        stations: Object.fromEntries(Object.entries(initialTwinLayout.stations).map(([id, station]) => [id, { ...station, ...stored.stations?.[id], relativeX: clampRelativeX(stored.stations?.[id]?.relativeX ?? station.relativeX) }])),
      }
    } catch {
      return initialTwinLayout
    }
  })

  useEffect(() => {
    localStorage.setItem('mes-digital-twin-layout-v4', JSON.stringify(layout))
  }, [layout])

  useEffect(() => {
    const syncLayout = (event: StorageEvent) => {
      if (event.key !== 'mes-digital-twin-layout-v4' || !event.newValue) return
      try {
        const stored = JSON.parse(event.newValue) as Partial<TwinLayout>
        setLayout((current) => ({
          ...current,
          ...stored,
          car: { ...current.car, ...stored.car, relativeX: clampRelativeX(stored.car?.relativeX ?? current.car.relativeX) },
          stations: Object.fromEntries(Object.entries(current.stations).map(([id, station]) => [id, {
            ...station,
            ...stored.stations?.[id],
            relativeX: clampRelativeX(stored.stations?.[id]?.relativeX ?? station.relativeX),
          }])),
        }))
      } catch {
...
function useSmoothedNumber(target: number | null, fallback: number, durationMs = 240) {
...
function applyLiveStationStates(layout: TwinLayout, live: TwinLiveState, maxPos: number): TwinLayout {
  return {
    ...
    stations: Object.fromEntries(Object.entries(layout.stations).map(([id, station]) => {
      ...
      return [id, {
        ...station,
        forward: state === undefined ? station.forward : state === 3,
        reverse: state === undefined ? station.reverse : state === 7,
        occupied: live.stationOccupied[index] === undefined ? station.occupied : live.stationOccupied[index] === true,
...
export function DigitalTwinModule({ stationOperationLabels = {} }: { stationOperationLabels?: StationOperationLabels }) {
  const [layout, setLayout] = useSharedTwinLayout()
    ...
    return {
      ...
      stations: Object.fromEntries(Object.entries(buildStations(maxPos, systemConfiguration.opRows)).map(([id, station]) => {
        ...
        return [id, {
          ...
          forward: previous?.forward ?? station.forward,
          reverse: previous?.reverse ?? station.reverse,
          occupied: previous?.occupied ?? station.occupied,
        }]
      })),
    }
  }, [layout, maxPos, systemConfiguration.opRows])
  const liveLayout = activeLayout
  const shuttleRevolving = liveLayout.car.revolving
  const shuttleMoving = liveLayout.car.moving
  const shuttleMovingDirection = liveLayout.car.movingDirection
  const carRelativeX = liveLayout.car.relativeX
      ...
      <section className="digital-twin-params">
        <ParamCard title="小车">
          <div className="digital-twin-car-row">
            <NumberField label="坐标" value={carRelativeX} onChange={(value) => setLayout((current) => ({ ...current, car: { ...current.car, relativeX: clampRelativeX(value) } }))} />
            <SelectField label="状态" value={shuttleRevolving ? '正反转' : shuttleMoving ? '移动' : '停止'} options={['停止', '正反转', '移动']} onChange={(value) => setLayout((current) => ({ ...current, car: { ...current.car, revolving: value === '正反转', moving: value === '移动' } }))} />
          </div>
        </ParamCard>

        <LanePanel title="上排工位" subtitle={topSubtitle}>
          {groupedStations.top.map(([id, station]) => (
            <StationRow key={id} code={station.opCode} state={station.forward ? 3 : station.reverse ? 7 : 0} x={station.relativeX} occupied={station.occupied} onChange={(next) => setLayout((current) => ({ ...current, stations: { ...current.stations, [id]: { ...current.stations[id], relativeX: next.x, forward: next.status === '正转', reverse: next.status === '反转', occupied: next.occupied } } }))} />
          ))}
        </LanePanel>
...
export function DigitalTwinWallboardGraphic({ stationOperationLabels = {}, flipped }: { stationOperationLabels?: StationOperationLabels; flipped?: boolean }) {
  ...
  const smoothPosition = useSmoothedNumber(live.shuttlePosition, liveLayout.car.relativeX)
  const shuttleState = live.shuttleState
  const renderedLayout = useMemo(() => buildWallboardTwinLayout({
    ...liveLayout,
    car: { ...liveLayout.car, relativeX: clampRelativeX(smoothPosition), revolving: shuttleState === 3 || shuttleState === 7, moving: shuttleState === 2 || shuttleState === 5, movingDirection: shuttleState === 2 ? 'right' : 'left' },
  }), [liveLayout, smoothPosition, shuttleState])
  return <DigitalTwinGraphic layout={renderedLayout} embedded stationOperationLabels={stationOperationLabels} flipped={flipped ?? systemConfiguration.isInterfaceFlipped} />
}
...
function StationRow({ code, state, x, occupied, onChange }: { code: string; state?: number; x: number; occupied: boolean; onChange: (next: { x: number; status: '停止' | '正转' | '反转'; occupied: boolean }) => void }) {
  const motion = state === 3 ? '正转' : state === 7 ? '反转' : '停止'
  return (
    <div className="digital-twin-station-row">
      <strong>{code}</strong>
      <NumberField label="坐标" value={x} onChange={(value) => onChange({ x: clampRelativeX(value), status: motion, occupied })} />
      <SelectField label="" value={motion} options={['停止', '正转', '反转']} onChange={(value) => onChange({ x, status: value as '停止' | '正转' | '反转', occupied })} />
      <label className="digital-twin-toggle compact"><input type="checkbox" checked={occupied} onChange={(event) => onChange({ x, status: motion, occupied: event.target.checked })} /><span>占用</span></label>
    </div>
  )
}

function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <label className="digital-twin-inline-field"><span>{label}</span><input type="number" min={0} max={RELATIVE_X_MAX} step={1} value={value} onChange={(event) => onChange(clampRelativeX(Number(event.target.value)))} /></label>
}

function SelectField({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) {
  return <label className="digital-twin-inline-field">{label ? <span>{label}</span> : null}<select value={value} onChange={(event) => onChange(event.target.value)}>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>
}
