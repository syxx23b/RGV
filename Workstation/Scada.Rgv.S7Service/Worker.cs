...
{
    private const int DefaultMaxPos = 20;
    private const int MaxPositionLimit = 48;
    private const int PositionRowSizeInBytes = MaxPositionLimit / 8;
    private const int StationRecordStrideInBytes = 6;
    private const int AndonRecordStrideInBytes = 4;
        ...
        lock (_lock)
        {
            foreach (var tag in _tags.Values.ToArray())
            {
                ...
                _tags[tag.Address] = tag with
                {
                    Value = saved.Value,
                    Quality = "Stale",
                    SourceTimestamp = saved.SourceTimestamp,
                    ServerTimestamp = saved.ServerTimestamp,
                    LastError = "已恢复最后一次 S7 快照。",
        ...
        try
        {
            ...
            try
            {
                ...
                foreach (var tag in tags)
                {
                    ...
                    if (tag.Name == "MaxPos" && int.TryParse(Convert.ToString(rawValue, CultureInfo.InvariantCulture), NumberStyles.Integer, CultureInfo.InvariantCulture, out var value) && value is >= 1 and <= MaxPositionLimit)
                    {
                        plcMaxPos = value;
                    }
                    var now = DateTimeOffset.UtcNow;
            ...
            if (plcMaxPos is int maxPos && maxPos != GetMaxPos())
            {
                ApplyConfiguration(_configurationStore.Read() with { MaxPos = maxPos });
                _logger.LogInformation("MaxPos updated from PLC to {MaxPos}.", maxPos);
            }
    ...
    private Dictionary<string, object> ReadBatchValues(IReadOnlyList<S7Tag> tags)
    {
        ...
        {
            var areaTags = group.ToArray();
            var parsedTags = areaTags.Select(tag => (tag, parsed: ParseAddress(tag.Address))).ToArray();
            ...
            {
                var index = item.parsed.offset - min;
                result[item.tag.Address] = DecodeValue(bytes, index, item.parsed.bit, item.tag.DataType);
            }
        }
        return result;
    }
    ...
    private static (DataType type, int db, int offset, int bit) ParseAddress(string address)
    {
        ...
        if (normalized.StartsWith("Q", StringComparison.Ordinal) && !normalized.StartsWith("QW", StringComparison.Ordinal))
        {
            var parts = normalized[1..].Split('.');
            return (DataType.Output, 0, int.Parse(parts[0], CultureInfo.InvariantCulture), int.Parse(parts[1], CultureInfo.InvariantCulture));
        }
        var match = Regex.Match(normalized, @"^(?:DB(?<db>\d+)\.DB(?<kind>[XW])(?<offset>\d+)(?:\.(?<bit>\d+))?|Q(?<kind2>[XW])(?<offset2>\d+)(?:\.(?<bit2>\d+))?)$");
        if (!match.Success) throw new FormatException($"无法解析 S7 地址：{address}");
        var isDb = match.Groups["db"].Success;
        var offset = int.Parse(isDb ? match.Groups["offset"].Value : match.Groups["offset2"].Value, CultureInfo.InvariantCulture);
        var bitText = isDb ? match.Groups["bit"].Value : match.Groups["bit2"].Value;
        var kind = isDb ? match.Groups["kind"].Value : match.Groups["kind2"].Value;
        return (isDb ? DataType.DataBlock : DataType.Output, isDb ? int.Parse(match.Groups["db"].Value, CultureInfo.InvariantCulture) : 0, offset, string.IsNullOrEmpty(bitText) ? -1 : int.Parse(bitText, CultureInfo.InvariantCulture));
    }
    ...
    private static Dictionary<string, S7Tag> CreateTags(int stationNumber, int maxPos)
    {
        var tags = new List<S7Tag>
        {
            ...
            new("MaxPos", "DB4.DBW104", "Int16", "Station", "ReadOnly"),
        ...
        var stationSignalByteOffset = 302 + (stationNumber - 1) * StationRecordStrideInBytes;
        foreach (var (name, bitOffset, access) in new[]
        {
            ("Insure", 0, "ReadWrite"), ("SBes", 1, "ReadOnly"), ("InPos", 2, "ReadOnly"),
            ("ForceR", 3, "ReadWrite"), ("ForceC", 4, "ReadWrite"),
        })
        {
            tags.Add(new S7Tag(name, $"DB4.DBX{stationSignalByteOffset}.{bitOffset}", "Bool", "Station", access));
        }

        var stationSignalByteOffset2 = 303 + (stationNumber - 1) * StationRecordStrideInBytes;
        foreach (var (name, bitOffset) in new[] { ("TurnR", 0), ("TurnC", 1), ("Mode0EQSend1EQCall", 2), ("Enable4Send", 3), ("Enable4Call", 4) })
        {
            tags.Add(new S7Tag(name, $"DB4.DBX{stationSignalByteOffset2}.{bitOffset}", "Bool", "Station", "ReadOnly"));
        }

        tags.Add(new S7Tag("OrderExist", $"DB4.DBX{327 + (stationNumber - 1) * StationRecordStrideInBytes}.5", "Bool", "Station", "ReadOnly"));
        tags.Add(new S7Tag("AndonCall", $"DB9.DBX{(stationNumber - 1) * AndonRecordStrideInBytes}.1", "Bool", "Andon", "ReadWrite"));
        tags.Add(new S7Tag("AndonState", $"DB9.DBW{2 + (stationNumber - 1) * AndonRecordStrideInBytes}", "Int16", "Andon", "ReadOnly"));
        for (var index = 1; index <= maxPos; index++)
        {
            var byteOffset = (stationNumber - 1) * PositionRowSizeInBytes + (index - 1) / 8;
            var bitOffset = (index - 1) % 8;
            tags.Add(new S7Tag($"EnableShowSend[{stationNumber},{index}]", $"DB2.DBX{byteOffset}.{bitOffset}", "Bool", "Station", "ReadOnly"));
            tags.Add(new S7Tag($"EnableShowCall[{stationNumber},{index}]", $"DB2.DBX{240 + byteOffset}.{bitOffset}", "Bool", "Station", "ReadOnly"));
            tags.Add(new S7Tag($"HMIPosSel[{stationNumber},{index}]", $"DB4.DBX{2 + byteOffset}.{bitOffset}", "Bool", "Station", "ReadWrite"));
